create or replace function private.list_client_application_details(
...
begin
  ...
  join public.client_daily_status as daily_status
    on daily_status.generator_id = application.generator_id
    and daily_status.status_date = application.public_date
    and daily_status.status = 'completed'
  ...
  join public.locations as location
    on location.id = daily_status.location_id
    and location.client_id = daily_status.client_id
  left join private.effective_power_readings as reading
    on reading.generator_id = application.generator_id
    and reading.occurred_at >= start_event.occurred_at
    and reading.occurred_at <= end_event.occurred_at
...
comment on function private.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
  ...
  'Implementação protegida da agenda do cliente por aplicação, com horário local e pico de potência no intervalo.';

create or replace function public.list_client_application_details(
  p_start_date date default null,
  p_end_date date default null,
  p_location_id uuid default null,
  p_cold_room_id uuid default null,
...
as $$
  select *
  from private.list_client_application_details(
    p_start_date,
    p_end_date,
    p_location_id,
    p_cold_room_id,
...
comment on function public.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
  ...
  'Agenda pública por aplicação: horários locais e maior potência medida entre o ligar e desligar; sem eventos, leituras ou identificadores técnicos.';

revoke all on function private.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
...
grant execute on function private.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
...
revoke all on function public.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
...
grant execute on function public.list_client_application_details(
  date,
  date,
  uuid,
  uuid,
